	...
	async resolve({ request }, { bookingId }) {
		...
		try {
			if (request.user && request.user.id) {
				...
				if (bookingData && bookingData.status === 'arrived') {
					...
					sendSocketNotification('serviceReviewed-' + bookingData.id, content);
					sendNotifications({ type: 'serviceReviewed', requestContent: content, userId, lang: requestLang, userType: 1 });
