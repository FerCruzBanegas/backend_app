...
const arrivedBooking = {
	...
	args: {
		...
		endLat: { type: new NonNull(FloatType) },
		endLng: { type: new NonNull(FloatType) },
		endLocation: { type: new NonNull(StringType) }
	},

	async resolve({ request }, { bookingId, endLat, endLng, endLocation }) {
		...
		try {
			if (request.user && request.user.id) {
				...
				if (bookingData && bookingData.status === 'approved') {
					...
					sendSocketNotification('arrivedLocation-' + bookingData.id, content);
					sendNotifications({ type: 'arrivedLocation', requestContent: content, userId, lang: requestLang, userType: 1 });
					...
					errorMessage = 'Oops! Unable to process this service. It looks like this service is already ';

					if (bookingData && bookingData.status === 'declined') {
