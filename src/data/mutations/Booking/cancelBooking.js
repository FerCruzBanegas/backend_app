...
import {
	...
	User, BookingCancelReason,
	SiteSettings, OrderItems,
	BookingReviewImage,
	SubCategory, Orders
} from '../../models';
	...
	async resolve({ request }, { bookingId, requestBy, reason }) {
		...
		try {
			if (request.user && request.user.id) {
				...
				let categoryId, nextpartnerId;
				let userLocationLat, userLocationLng, userLocation;
				let errorMessage, isPartner = false, id;
				...
				const restrictedStatus = ['cancelledByUser', 'cancelledByPartner', 'completed', 'expired', 'started'];
				const requestProfileParams = ['preferredLanguage', 'preferredCurrency', 'address', 'lat', 'lng',
					'preferredLocation', 'preferredLat', 'preferredLng', 'firstName', 'picture', 'lastName'];

				const userProfileData = await getUserProfileData(bookingData.userId, requestProfileParams);

				if (bookingData && restrictedStatus.indexOf(bookingData.status) >= 0) {
					isPartner = (bookingData.partnerId === userId) ? true : false;
					if (bookingData.status === 'cancelledByUser') {
						errorMessage = 'Oops! It looks like the service is already cancelled by ';
						errorMessage = errorMessage + ((isPartner) ? 'the user.' : ' you.');
					} else if (bookingData.status === 'cancelledByPartner') {
						errorMessage = 'Oops! It looks like the service is already cancelled by ';
						errorMessage = errorMessage + ((isPartner) ? 'you.' : ' the partner.');
					} else if (bookingData.status === 'completed') {
						errorMessage = 'Oops! It looks like the service is already completed.';
					} else {
					...
					if (requestBy === 'cancelledByPartner') {
						...
						if (nearestPartner && nearestPartner.status === 200 && nearestPartner.result) {
							...
							const partnerProfileData = await getUserProfileData(nextpartnerId, requestProfileParams);
					...
					id = (requestBy === 'cancelledByPartner') ? bookingData.userId : bookingData.partnerId;

					const partnerProfileData = await getUserProfileData(bookingData.partnerId, requestProfileParams);
					...
					let content = {
						...
						bookingId,
						bookingStatus: requestBy
					};

					sendSocketNotification('cancelService-' + bookingId, content);
					if (requestBy === 'cancelledByPartner') sendNotifications({ type: 'cancelledByPartner', requestContent: content, userId: bookingData.userId, lang: requestLang, userType: 1 });
					if (requestBy === 'cancelledByUser') sendNotifications({ type: 'cancelledByUser', requestContent: content, userId: bookingData.partnerId, lang: requestLang, userType: 2 });
